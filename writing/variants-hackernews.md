...
```
Author here. Three things I'd flag before anyone reads it as a security claim.
...
Second, and this is the part I'd want scrutinised: all six of those bypasses
...
Third, and it undercuts the point I just made, so it should be here rather than
in a footnote. A seventh route turned up the day after those sessions, and not
from an attack at all — I wrote the policy format up as a versioned spec, and
stating precisely what a rule means surfaced a bug in the matcher. `git push --force` is denied;
`git -c core.pager=cat push --force` is not, because the subcommand test reads
the first non-flag argument and `-c` puts its value there. That one is not a
mapping gap. It's in a rule I wrote and believed I understood. It's open on
purpose, because closing it means knowing which flags of which commands take a
separate value, and the only thing that could supply that on demand is a model,
which is exactly what isn't allowed near a runtime decision — so it's specified
and pinned as a conformance case instead, which makes any fix a version bump
rather than a quiet matcher edit.
...
Around 380 words. It concedes the weakest point in the first paragraph and
contradicts its own thesis in the third, which is the only reliable way to
...
- **"This is just a denylist with extra steps."** Partly fair for the shell rules,
  ...
  string denylist catches one of them. Do not overclaim here: the seventh route
  is the same rule failing on `git -c`, and the comment already says so.
